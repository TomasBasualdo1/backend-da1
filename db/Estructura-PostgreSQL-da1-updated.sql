...
CREATE TABLE public.articulos (
  ...
  estado character varying DEFAULT 'pendiente'::character varying CHECK (estado::text = ANY (ARRAY['pendiente'::character varying, 'interesado'::character varying, 'en_transito'::character varying, 'en_inspeccion'::character varying, 'aprobado'::character varying, 'rechazado'::character varying, 'devuelto'::character varying]::text[])),
  motivo_rechazo character varying,
  ...
  direccion_inspeccion character varying,
  instrucciones_envio character varying,
  acepta_cargo_devolucion boolean DEFAULT false,
  costo_devolucion numeric CHECK (costo_devolucion IS NULL OR costo_devolucion >= 0),
  fecha_envio_fisico timestamp with time zone,
